...
<task type="auto" tdd="true">
  <name>Task 7: Add export UI to tenant detail and product search views</name>
  ...
  <behavior>
    Test 1: Tenant detail view has "Export Sync Logs" button
    Test 2: Product search view has "Export Catalog" button
    Test 3: Export button shows loading state during job processing
    Test 4: Export button polls job status and shows download link when ready
    Test 5: Download link opens signed URL in new tab
    Test 6: Toast notification appears when export is ready
  </behavior>
  <action>
    Add export UI components to views:

    **Tenant Detail View** (resources/views/dashboard/tenants/show.blade.php):
    - Add "Export Sync Logs" button above sync log table
    - Include filter form (date range, status) before export button
    - Add loading state: spinner while job processing
    - Add download section: appears when job completes, shows download button
    - Use Alpine.js for state management (loading, completed, downloadUrl)

    **Product Search View** (resources/views/dashboard/tenants/products.blade.php):
    - Add "Export Catalog" button above product search
    ...
    **JavaScript** (public/js/dashboard.js):
    ...
    ```javascript
    exportSyncLogs(tenantId, filters, format) {
        return fetch('/api/v1/exports/sync-logs', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            this.pollExportStatus(data.data.job_uuid);
            return data;
        });
    }

    exportProductCatalog(tenantId) {
        return fetch(`/api/v1/exports/products`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    pollExportStatus(jobUuid) {
        const interval = setInterval(() => {
            fetch(`/api/v1/job-statuses/${jobUuid}`)
                .then(response => response.json())
                .then(data => {
                    if (data.data.status === 'completed') {
                        clearInterval(interval);
                        this.showDownloadLink(jobUuid);
                        this.showToast('Export ready!');
                    } else if (data.data.status === 'failed') {
                        clearInterval(interval);
                        this.showError('Export failed: ' + data.data.error_message);
                    }
                });
        }, 2000);
    }

    showDownloadLink(jobUuid) {
        fetch(`/api/v1/exports/${jobUuid}`)
            .then(response => response.json())
            .then(data => {
    ...
    Key points:
    - Export buttons dispatch jobs via API
    - Poll job status every 2 seconds
    - Show download link when job completes
    - Toast notification for export ready
    - Error handling for failed exports
  </action>
  <verify>
    <automated>MISSING — Wave 0 must create UI test stub first</automated>
  </verify>
  <done>Export UI added to tenant detail and product search views with loading states, download links, and toast notifications</done>
</task>
